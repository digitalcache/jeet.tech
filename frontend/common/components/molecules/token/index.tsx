  ...
  return (
    ...
    >
      <div className={`transition duration-200 px-4 py-4 z-10 relative w-full`}>
        ...
        <div className="text-neutral-600 text-sm mt-2 leading-4">
          {idea.description.replaceAll('$comma$', ',')}
        </div>
        <div className="mt-2">
          <div className="flex flex-col w-max">
            <span className="text-sm text-neutral-500">{ideaCardCopy.raised}</span>
            <span className="text-sm text-neutral-700 font-medium">{fundingRaised ? fundingRaised.toFixed(3).replace(/[.,]000$/, "") : 0} {process.env.NEXT_PUBLIC_CURRENCY || ''}</span>
          </div>
        </div>
      </div>
    </Link>
